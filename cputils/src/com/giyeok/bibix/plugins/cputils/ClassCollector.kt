...
class ClassCollector(val classpaths: List<Path>, allDeps: List<Path>) {
  ...
  private fun loadClassesFromDirectory(basePath: Path) {
  ...
  private fun loadClassesFromJar(path: Path) {
    path.inputStream().buffered().use { fis ->
