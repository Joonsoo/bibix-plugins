...
class Run {
  fun run(context: ActionContext): BuildRuleReturn {
    ...
    val targetCps = deps.values.flatMap { dep ->
      val depCps =
        ((dep as DataClassInstanceValue).fieldValues.getValue("cps") as SetValue)
    ...
    return BuildRuleReturn.evalAndThen(
      ...
      classPaths as DataClassInstanceValue
      val cps = (classPaths.fieldValues.getValue("cps") as SetValue).values.map {
        (it as PathValue).path
      }
      val res = classPaths.fieldValues.getValue("res") as SetValue // set<path>
